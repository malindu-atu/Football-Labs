    ...
    <div style={card} className="rounded-2xl p-6 mb-6">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <p className="text-gray-400 text-xs mt-0.5">Claude will auto-fill the student details from the document</p>
        </div>
      </div>
      ...
      <div
        ...
        style={{
          border: `2px dashed ${dragOver ? "#00E5CC" : "rgba(0,229,204,0.25)"}`,
          backgroundColor: dragOver ? "rgba(0,229,204,0.05)" : "rgba(0,0,0,0.2)",
          transition: "all 0.2s",
          cursor: "pointer",
        }}
      ...
      >
        ...
        {preview ? (
          <img src={preview} alt="Enrollment document preview"
            className="max-h-52 rounded-lg object-contain border border-white/10" />
        ) : (
          <>
...
export default function Kids() {
  ...
  return (
    <div style={pageWrapper} className="p-4 sm:p-6 lg:p-8">
      ...
      {/* Step 1: Scan enrollment document */}
      {step === "scan" && (
        <EnrollmentScanner
          onExtracted={handleExtracted}
          onSkip={() => { setForm(EMPTY_FORM); setStep("form"); }}
        />
      )}
        ...
        <div style={card} className="rounded-2xl p-4 sm:p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input style={input} className="w-full rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
                placeholder="Parent / guardian name" value={form.parent_name}
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block">Enrollment Date</label>
              <input style={input} className="w-full rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/30"
                type="date" value={form.enrollment_date}
      ...
      <div style={card} className="rounded-2xl overflow-hidden">
          ...
          {kids.map(k => (
            <div key={k.id} style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }} className="p-4">
              ...
              <div className="ml-12 grid grid-cols-2 gap-1 text-xs text-gray-400">
                ...
                {k.enrollment_date && <span className="col-span-2">📅 Enrolled: {k.enrollment_date}</span>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="hidden sm:block overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {loading && <tr><td colSpan={6} className="p-8 text-center text-gray-500">Loading students…</td></tr>}
              {kids.map(k => (
